    ...
    return (
        <div className="animate-fade-in">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
                <h1 className="title" style={{ fontSize: '1.8rem', margin: 0 }}>📋 ประวัติการตรวจงาน</h1>
                <select
                ...
                >
                    ...
                    <option value="PENDING">รอตรวจ ⏳ ({pendingCount})</option>
                    <option value="APPROVED">อนุมัติแล้ว ✅ ({approvedCount})</option>
                    <option value="REJECTED">ถูกปฏิเสธ ❌ ({rejectedCount})</option>
                </select>
            </div>
            ...
            <div style={{ display: 'flex', gap: '12px', marginBottom: '24px', flexWrap: 'wrap' }}>
                {[
                    { label: 'รอตรวจ', value: pendingCount, color: '#ffb703' },
                    { label: 'อนุมัติแล้ว', value: approvedCount, color: 'var(--success)' },
                    { label: 'ปฏิเสธ', value: rejectedCount, color: 'var(--danger)' },
                ].map(stat => (
                    <div key={stat.label} className="card" style={{ flex: 1, minWidth: '100px', textAlign: 'center', padding: '12px', borderTop: `3px solid ${stat.color}` }}>
            ...
            {loading ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    {filtered.map(sub => {
                        ...
                        const playerName = sub.user?.displayName || sub.user?.username || 'Player';
                        return (
                            <div key={sub.id} className="card" style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '8px' }}>
                                    <div>
                                        <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '4px' }}>
                                            <span style={{ fontSize: '0.8rem', background: 'var(--primary-light)', color: 'var(--primary-dark)', padding: '2px 8px', borderRadius: '12px', fontWeight: 600 }}>
                                                👤 {playerName}
                                            </span>
                                            <span className="badge" style={{ background: '#eee', color: '#666' }}>{sub.task.type}</span>
                                        </div>
                                        <h3 style={{ fontWeight: 600, fontSize: '1rem', margin: 0 }}>{sub.task.title}</h3>
                                        <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', margin: '4px 0 0' }}>
                                            ส่งเมื่อ: {formatDate(sub.createdAt)}
                                            {sub.status !== 'PENDING' && ` • ตรวจเมื่อ: ${formatDate(sub.updatedAt)}`}
                                        </p>
                                    </div>
