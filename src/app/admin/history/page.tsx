    ...
    return (
        <div className="animate-fade-in">
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '24px' }}>
                <button style={tabStyle(tab === 'submissions')} onClick={() => setTab('submissions')}>📝 การตรวจงาน</button>
                                    ...
                                    <div key={sub.id} className="card" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                            <div>
                                                ...
                                                <p style={{ color: 'var(--text-muted)' }}>ส่งโดย: {playerName}</p>
                                                <p style={{ fontSize: '0.85rem', color: '#888' }}>{new Date(sub.createdAt).toLocaleString('th-TH')}</p>
                                            </div>
                ...
                <>
                    {loading ? (
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                            {redeemLogs.map(log => {
                                const playerName = log.user?.displayName || log.user?.username || '';
                                return (
